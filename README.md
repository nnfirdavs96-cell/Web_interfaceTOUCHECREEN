...
- **Сайдбар с секциями** — 5 редакционных групп (`/ обзор`, `/ структура`, `/ доступ`, `/ активность`, `/ администрирование`) с collapsible-переключателями ChevronDown. Состояние секций сохраняется в `localStorage`. Автоматическое раскрытие секции при навигации к её маршруту. Сайдбар всегда виден на десктопе (md+), скрыт на мобильных.
...
**Где видно:** дашборд (glow KPI-карточки с градиентными цифрами, недельная диаграмма со светящимися столбцами), сайдбар (стеклянный фон, 5 collapsible-секций, светящийся active-индикатор), топбар (рабочий поиск, cobalt-аватар, переключатель темы), логин (particle-cloud + градиентный герой).
